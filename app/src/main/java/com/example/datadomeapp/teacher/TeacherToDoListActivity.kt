...
// Same TaskItem data class as student side
...
class TeacherToDoListActivity : AppCompatActivity() {
    ...
    private var allTasks = listOf<TeacherTaskItem>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.student_to_do_list) // Using same layout as student
    ...
    private fun loadTasks() {
        ...
        Log.d("TeacherToDoListDebug", "Attempting to load tasks for Teacher ID: $userId")
        ...
        db.collection("teachers").document(userId)
            ...
            .addOnSuccessListener { result ->
                ...
                Log.d("TeacherToDoListDebug", "Query successful. Found ${result.size()} tasks.")

                allTasks = result.documents.mapNotNull { doc ->
                    doc.toObject(TeacherTaskItem::class.java)?.copy(
                        taskId = doc.id,
            ...
            .addOnFailureListener { e ->
                Log.e("TeacherToDoListDebug", "Firestore Query Failed!", e)
    ...
    // ----------------------------------------------------
    private fun createTaskView(task: TeacherTaskItem): View {
        ...
        if (!task.done) {
            ...
            if (task.date != "No Date Set" && task.time != "No Time Set") {
                try {
                    ...
                    Log.e("TeacherToDoList", "Failed to parse date/time: ${task.date} ${task.time}", e)
                }
            }
        }
    ...
    private fun saveTask(
        ...
        if (date != "No Date Set" && time != "No Time Set") {
            try {
                ...
                Log.e("TeacherToDoList", "Timestamp parsing failed.", e)
            }
        }
        ...
        val taskRef = db.collection("teachers").document(userId).collection("tasks")
