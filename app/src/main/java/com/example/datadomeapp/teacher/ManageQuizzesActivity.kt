...
// -------------------------------------------------------------------------------------
    ...
    // Sa loob ng ManageQuizzesActivity.kt
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (child in snapshot.children) {
                        ...
                        // 🎯 CRITICAL: I-extract ang assignmentId mula sa database
                        ...
                        // --- CONSTRUCT QUIZ OBJECT (Para sa Status Check) ---
                        ...
                        // 3. GLOBAL VIEW FILTER (FIXED LOGIC)
                        if (isGlobalView) {
                            // Tanging Published Quizzes (Ongoing, Scheduled, Finished) at
                            // Quizzes na may schedule (Drafts with time set) lang ang isasama.

                            // HUWAG ISAMA ang UNPUBLISHED DRAFTS na WALANG schedule.
                            if (!quiz.isPublished && quiz.scheduledDateTime == 0L) {
                                continue
                            }
                        }
                        // Tandaan: Sa Class View, isasama lahat ng status (Draft, Scheduled, Ongoing, Finished)
...
// -------------------------------------------------------------------------------------
    ...
    private fun togglePublish(quiz: Quiz) {
        ...
        if (newStatus == true) { // PUBLISH
            ...
            db.child("quizzes").child(quizId).child("published").setValue(true)
                .addOnSuccessListener {
    ...
    private fun showStartTimePicker(quiz: Quiz, scheduleCalendar: Calendar, isCalledAfterPublish: Boolean) {
        ...
        val timePicker = TimePickerDialog(
            ...
            { _, startHour, startMinute ->
                ...
                val MIN_START_DELAY_MS = 5 * 60 * 1000L // 5 minutes in milliseconds

                if (startTime < currentTime + MIN_START_DELAY_MS){
                    Toast.makeText(this, "Start time must be at least 5 minutes from the current time.", Toast.LENGTH_LONG).show()
                    showStartTimePicker(quiz, scheduleCalendar, isCalledAfterPublish)
...
// -------------------------------------------------------------------------------------
    ...
    private fun openQuizEditor(quiz: Quiz?) {
        if (quiz != null && (isQuizOngoing(quiz) || isQuizFinished(quiz))) {
            ...
            Toast.makeText(this, "View Quiz Results feature coming soon!", Toast.LENGTH_SHORT).show()
            return
        }
